...
void print_stack_itop(); //infix to postfix 변환과정 스택상태 출력 
void print_stack_eval(); //postfix 계산과정 스택상태 출력
...
int main(void)
{
    ...
    printf("\n");
    printf("Status of the stack during evaluation:\n");
...
int getop(char s[])
{
    ...
    while ((s[0] = c = getchar()) == ' ' || c == '\t')
        ;
    s[1] = '\0';
    if (!isdigit(c)) //입력받은 문자가 숫자가 아닐 때
        return c;   	
    i = 0;
    if (isdigit(c)) //첫글자가 숫자면 계속 숫자 입력받을때까지 s에 저장 
